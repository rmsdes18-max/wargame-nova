...
function renderStatsMatchingV3(results){
  ...
  html += '<style id="stats-v3-css">'
    ...
    +'.sm3-matched-list{background:#1a1a1a;border:1px solid #333;border-radius:10px;margin-bottom:16px;margin-top:-8px;display:none;max-height:250px;overflow-y:auto;}'
    ...
    +'.sm3-attention-section{background:#1a1a1a;border:1px solid #333;border-radius:10px;overflow:hidden;margin-bottom:24px;}'
    +'.sm3-attention-header{background:linear-gradient(135deg,rgba(240,192,64,.15) 0%,rgba(240,192,64,.05) 100%);padding:14px 20px;display:flex;align-items:center;gap:10px;border-bottom:1px solid #333;}'
    +'.sm3-attention-header.done{background:linear-gradient(135deg,rgba(62,207,110,.15) 0%,rgba(62,207,110,.05) 100%);}'
    +'.sm3-attention-header.done .sm3-att-title{color:#3ecf6e;}'
    +'.sm3-att-title{font-size:14px;font-weight:600;color:#f0c040;}'
    +'.sm3-att-count{background:#f0c040;color:#000;font-size:11px;font-weight:700;padding:2px 8px;border-radius:10px;}'
    ...
    +'.sm3-attention-ocr{font-family:monospace;font-size:15px;font-weight:600;color:#f0c040;min-width:140px;}'
    +'.sm3-attention-stats{display:flex;gap:16px;font-size:13px;font-family:monospace;}'
    +'.sm3-attention-stats .k{color:#e84040;} .sm3-attention-stats .d{color:#c9a06a;} .sm3-attention-stats .h{color:#3ecf6e;}'
    +'.sm3-action-row{display:flex;align-items:center;gap:10px;}'
    +'.sm3-action-row label{font-size:12px;color:#888;min-width:90px;}'
    +'.sm3-member-dropdown{flex:1;max-width:260px;background:#252525;border:1px solid #333;border-radius:6px;padding:10px 14px;color:#e0e0e0;font-size:13px;cursor:pointer;}'
    +'.sm3-member-dropdown:focus{outline:none;border-color:#f0c040;}'
    +'.sm3-btn-confirm{background:#f0c040;color:#000;border:none;border-radius:6px;padding:10px 20px;font-size:13px;font-weight:600;cursor:pointer;transition:all .15s;}'
    +'.sm3-btn-confirm:hover{background:#e0b030;}'
    +'.sm3-btn-confirm:disabled{background:#252525;color:#888;cursor:not-allowed;}'
    +'.sm3-toggle-options{display:flex;gap:16px;margin-top:8px;padding-top:12px;border-top:1px dashed #333;}'
    +'.sm3-toggle-option{display:flex;align-items:center;gap:6px;font-size:12px;color:#888;cursor:pointer;padding:6px 12px;border-radius:6px;transition:all .15s;}'
    +'.sm3-toggle-option:hover{background:#252525;color:#e0e0e0;}'
    +'.sm3-toggle-option.active{background:rgba(240,192,64,.15);color:#f0c040;}'
    +'.sm3-toggle-option input{accent-color:#f0c040;}'
    +'.sm3-new-member-form{display:none;margin-top:12px;padding:14px;background:#252525;border-radius:8px;border:1px solid #333;}'
    ...
    +'.sm3-form-row label{font-size:12px;color:#888;min-width:70px;}'
    +'.sm3-form-input{flex:1;max-width:200px;background:#1a1a1a;border:1px solid #333;border-radius:6px;padding:8px 12px;color:#e0e0e0;font-size:13px;}'
    +'.sm3-form-input:focus{outline:none;border-color:#f0c040;}'
    +'.sm3-form-select{background:#1a1a1a;border:1px solid #333;border-radius:6px;padding:8px 12px;color:#e0e0e0;font-size:13px;min-width:100px;}'
    +'.sm3-form-checkbox{display:flex;align-items:center;gap:8px;font-size:12px;color:#888;}'
    +'.sm3-form-checkbox input{accent-color:#f0c040;width:16px;height:16px;}'
    +'.sm3-suggestion-hint{margin-top:8px;font-size:11px;color:#888;display:flex;align-items:center;gap:6px;}'
    +'.sm3-suggestion-hint .match{color:#3ecf6e;font-weight:600;cursor:pointer;text-decoration:underline;text-decoration-style:dotted;}'
    +'.sm3-suggestion-hint .match:hover{text-decoration-style:solid;}'
    +'.sm3-resolved-badge{display:inline-flex;align-items:center;gap:6px;background:rgba(62,207,110,.2);color:#3ecf6e;padding:8px 14px;border-radius:6px;font-size:13px;font-weight:600;}'
    +'.sm3-resolved-badge .new-tag{background:#f0c040;color:#000;font-size:9px;padding:2px 6px;border-radius:4px;margin-left:4px;}'
    +'.sm3-all-done{text-align:center;padding:30px 20px;display:none;}'
    +'.sm3-all-done.show{display:block;}'
    +'.sm3-all-done h3{font-size:16px;font-weight:600;color:#3ecf6e;margin-bottom:4px;}'
    +'.sm3-all-done p{font-size:13px;color:#888;}'
    +'.sm3-party-preview{background:#1a1a1a;border:1px solid #333;border-radius:10px;margin-bottom:24px;}'
    +'.sm3-party-preview-header{padding:14px 20px;border-bottom:1px solid #333;display:flex;align-items:center;gap:10px;}'
    +'.sm3-party-preview-header h3{font-size:14px;font-weight:600;color:#e0e0e0;}'
    +'.sm3-party-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1px;background:#333;}'
    +'.sm3-party-card{background:#1a1a1a;padding:14px;}'
    +'.sm3-party-card-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #333;}'
    ...
    +'.sm3-party-member .stats{font-family:monospace;font-size:10px;color:#888;}'
    +'.sm3-party-member.new-match{background:rgba(240,192,64,.1);margin:0 -8px;padding:4px 8px;border-radius:4px;}'
    +'.sm3-footer{display:flex;justify-content:space-between;align-items:center;padding-top:16px;border-top:1px solid #333;}'
    ...
    +'.sm3-btn-secondary{background:transparent;border:1px solid #333;color:#888;border-radius:8px;padding:14px 20px;font-size:13px;cursor:pointer;}'
    +'.sm3-btn-secondary:hover{border-color:#888;color:#e0e0e0;}'
  ...
  html += '<div class="sm3-matched-banner" onclick="toggleMatchedV3()">'
    +'<div style="display:flex;align-items:center;gap:12px;">'
      ...
      +'<div>'
        +'<h3 style="font-size:15px;font-weight:600;color:#3ecf6e;margin:0 0 2px 0;">'+results.matched.length+' players matched automatically</h3>'
        +'<p style="font-size:12px;color:#888;margin:0;">Stats linked to roster members</p>'
      +'</div>'
    +'</div>'
  ...
  results.matched.forEach(function(m, mi){
    ...
    html += '<div class="sm3-matched-item">'
      ...
      +'<span style="color:#888;font-size:11px;font-family:monospace;margin-left:auto;">'
  ...
  if(results.matched.length > maxShow){
    html += '<div style="padding:10px 16px;text-align:center;color:#888;font-size:12px;">+ '+(results.matched.length - maxShow)+' more players</div>';
  }
  html += '</div>';
  ...
  html += '<div class="sm3-footer">'
    ...
    +'<div style="display:flex;gap:16px;align-items:center;">'
      +'<span style="font-size:12px;color:#888;display:flex;align-items:center;gap:6px;">&#128190; New aliases saved automatically</span>'
...
function skipItemV3(idx){
  ...
  action.innerHTML = '<span class="sm3-resolved-badge" style="background:rgba(136,136,136,.2);color:#888;">&#9193;&#65039; Skipped</span>';
...
function editStatV3(matchIdx, field, spanEl){
  ...
  input.type = 'number';
  input.value = curVal;
  input.style.cssText = 'width:60px;background:#252525;border:1px solid #f0c040;border-radius:4px;color:#e0e0e0;font-size:11px;font-family:monospace;padding:2px 4px;text-align:right;';
  var commit = function(){
