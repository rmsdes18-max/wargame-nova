  ...
  apiGet('/api/users', {fallback: []}).then(function(users){
      ...
      users.forEach(function(u){
        var avatarHtml = u.avatar
          ...
          : '<div style="width:32px;height:32px;border-radius:50%;background:var(--bg3);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;color:var(--text-muted);flex-shrink:0;">'+escHtml(u.username.charAt(0).toUpperCase())+'</div>';
      ...
      document.getElementById('users-list').innerHTML = html || '<div style="color:var(--muted);font-size:12px;">No users yet</div>';
    });
}
...
function showOnboardForm(type){
  ...
  document.getElementById('onboard-create').style.borderColor = type==='create' ? 'var(--gold)' : '';
  document.getElementById('onboard-join').style.borderColor = type==='join' ? 'var(--gold)' : '';
  ...
  el.innerHTML = _guilds.map(function(g){
    return '<div onclick="enterGuild({id:\''+g.id+'\',name:\''+escHtml(g.name)+'\'})" style="background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:16px 20px;margin-bottom:10px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:border-color .15s;" onmouseover="this.style.borderColor=\'var(--gold)\'" onmouseout="this.style.borderColor=\'\'">'
      ...
      +'<span style="font-size:10px;padding:2px 8px;border-radius:3px;font-weight:600;'
        +(g.tier==='pro'?'background:rgba(240,192,64,.15);color:var(--gold);':'background:rgba(255,255,255,.06);color:var(--text-muted);')
...
async function initGuildSettings(){
  ...
  try{
    ...
    tierEl.style.background = data.tier==='pro'?'rgba(240,192,64,.15)':'rgba(255,255,255,.06)';
    tierEl.style.color = data.tier==='pro'?'var(--gold)':'var(--text-muted)';
    ...
    (data.members||[]).forEach(function(m){
      var avatarHtml = m.avatar
        ...
        : '<div style="width:32px;height:32px;border-radius:50%;background:var(--bg3);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;color:var(--text-muted);flex-shrink:0;">'+escHtml(m.username.charAt(0).toUpperCase())+'</div>';
...
function copyInviteCode(){
  ...
  navigator.clipboard.writeText(code).then(function(){
    ...
    setTimeout(function(){ document.getElementById('gs-invite-code').style.color='var(--gold)'; },1000);
  });
}
